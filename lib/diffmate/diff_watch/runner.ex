defmodule Diffmate.DiffWatch.Runner do
  @moduledoc """
  Owns the app-server process and sends diff updates as turns.
  """

  use GenServer
  require Logger

  alias Diffmate.AppServerCommon

  @topic "diffmate:events"
  @port_line_bytes 1_048_576
  @non_interactive_answer "This is a non-interactive session. Operator input is unavailable."

  @meta_prompt "You are a partner to the user. The user sends messages to you via git diff output as the communication medium. Just respond directly and naturally. Do not treat this as a request to edit files unless the user explicitly asks."
  ...
  def init(_opts) do
    ...
    cwd = Diffmate.Config.config_file_path() |> Path.dirname()
  ...
  def handle_info(:do_initialize, state) do
    state =
      AppServerCommon.send_rpc(state, "initialize", %{
        ...
        clientInfo: %{name: "diffmate", title: "Diffmate", version: "0.1.0"}
      })
  ...
  def handle_info({:user_compact, _}, state) do
    Logger.warning("[Diffmate.Runner] /compact received but status=#{state.status}, ignoring")
    {:noreply, state}
  end
  ...
  defp decode_and_process(line, state) do
    AppServerCommon.decode_and_process(line, state, &process_message/2, "[Diffmate.Runner]")
  end
  ...
  defp handle_notification("turn/completed", params, %{status: :compacting_summary} = state) do
    case get_in(params, ["turn", "status"]) do
      s when s in ["failed", "interrupted"] ->
        Logger.error("[Diffmate.Runner] compact summary turn ended with status: #{s}")
  ...
  defp handle_notification("item/commandExecution/requestApproval", params, state) do
    AppServerCommon.send_response(state, params["id"], %{
      decision: command_approval_decision(state.approval_policy)
    })
  ...
  defp handle_notification("execCommandApproval", params, state) do
    AppServerCommon.send_response(state, params["id"], %{
      decision: legacy_approval_decision(state.approval_policy)
    })
  ...
  defp handle_notification("item/tool/requestUserInput", params, state) do
    case AppServerCommon.build_non_interactive_answers(params, @non_interactive_answer) do
      ...
      :error ->
        Logger.warning("[Diffmate.Runner] item/tool/requestUserInput: cannot build answers")
    end
  ...
  defp handle_notification("turn/failed", params, state) do
    Logger.error("[Diffmate.Runner] turn/failed: #{inspect(params)}")
    state = %{state | status: :idle, pending_method: nil}
    flush_pending(state)
  end

  defp handle_notification("turn/cancelled", params, state) do
    Logger.warning("[Diffmate.Runner] turn/cancelled: #{inspect(params)}")
    state = %{state | status: :idle, pending_method: nil}
    flush_pending(state)
  end
  ...
  defp handle_notification("turn/" <> _ = method, params, state) do
    if AppServerCommon.input_required?(params) do
      Logger.warning(
        "[Diffmate.Runner] turn input required (method=#{method}), treating as turn end"
      )
  ...
  def command_approval_decision("always"), do: "acceptForSession"
  def command_approval_decision(_approval_policy), do: "reject"

  def legacy_approval_decision("always"), do: "approved_for_session"
  def legacy_approval_decision(_approval_policy), do: "denied"
